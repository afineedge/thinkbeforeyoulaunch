...
function _languageFiles_getDirs() {
  ...
  static $isBuggyPHP;
  if (!isset($isBuggyPHP)) { // cache result so we're not checking version on every call
    $buggy52branch = version_compare(phpversion(), '5.2.14', '<');  // any PHP less then 5.2.14
    $buggy53branch = version_compare(phpversion(), '5.3.0', '>=') && version_compare(phpversion(), '5.3.3', '<'); // PHP 5.3.0 to PHP 5.3.2
