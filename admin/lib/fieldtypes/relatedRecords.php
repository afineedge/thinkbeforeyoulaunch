...
// NOTE: edit menus - we won't have record number for new records, so save record with ajax to get record number by calling edit_functions.js:saveRedirectAndReturn
function relatedRecordsButton($label, $url, $addReturnUrl = true) {
  
  // get menu type
  $isViewMenu = @$GLOBALS['action'] == 'view';  // view menus - we always have record number so just redirect

  // get target url
  $recordNumOrPlaceholder = $isViewMenu ? intval($_REQUEST['num']) : '###';  // ### gets replaced by saveRedirectAndReturn() in edit_functions.js
  if ($isViewMenu)   { $url  = str_replace('###', intval($_REQUEST['num']), $url); } // replace any occurances of ### in $url (in case it's used in multiple places)
  if ($addReturnUrl) { $url .= "&returnUrl=" .urlencode(thisPageUrl(array('num' => $recordNumOrPlaceholder), true)); }
  
  // get onclick
  if ($isViewMenu) { $onclick = htmlencode('window.location="' .jsEncode($url). '"; return false;'); } // no need to save on view menus and we know the record number, so just redirect
  else             { $onclick = htmlencode('saveRedirectAndReturn("' .jsEncode($url). '"); return false;'); } // call saveRedirectAndReturn() to save record, get record num, and replace ### with real record num in links
