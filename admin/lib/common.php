...
// Usage: $absPath = absPath('myfile.jpg', dirname(__FILE__));  // get path relative to path of current php script
...
function _displayNotificationType($type, $message) {
  ...
  print '<a class="close" href="#"><img alt="close" title="Close this notification" src="'.CMS_ASSETS_URL.'/3rdParty/SimplaAdmin/images/icons/cross_grey_small.png"/></a>';
...
function _sendMessage_swiftMailer($options) {
  ...
  try {
    ...
    @trigger_error($e->getMessage(), E_USER_NOTICE);  // log error
    $errors .= $e->getMessage() . "\n";               // return error
  }  
...
function getCurrentUserFromCMS() {
  ...
  if ($cmsUser['num']) { // 2.64 - only add if user found
