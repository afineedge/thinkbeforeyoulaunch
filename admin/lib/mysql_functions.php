...
function mysql_getMysqlSetValues($columnsToValues) {
  ...
  if (is_array($columnsToValues)) {
    foreach ($columnsToValues as $column => $value) {
      ...
      if (!preg_match('/^([\w\-]+)$/i', $column)) { die(__FUNCTION__. ": Invalid column name '" .htmlencode($column). "', contains disallowed chars!"); } // error checking: whitelist column chars to prevent sql injection
